...
if [[ "$(uname -s)" == "Darwin" ]]; then
  if [[ "$(uname -m)" == "arm64" ]] || [[ "$(sysctl -in sysctl.proc_translated)" == "1" ]]; then
    if command -v /usr/local/bin/brew >/dev/null 2>&1; then
      arch -x86_64 /usr/local/bin/brew bundle dump --file="$SCRIPT_PATH/homebrew/Brewfile.intel" --force
    fi

    if command -v /opt/homebrew/bin/brew >/dev/null 2>&1; then
      arch -arm64e /opt/homebrew/bin/brew bundle dump --file="$SCRIPT_PATH/homebrew/Brewfile" --force
    fi
  else
